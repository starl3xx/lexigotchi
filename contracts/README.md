...
| **Letters** | 52-id ERC-1155 (lowercase `i`=`i`, uppercase `i`=`26+i`). 100%-lowercase demand-mirrored draws with per-letter caps; commit→server-signed reveal (no expiry, so fees are never forfeited). Pack-of-5 + a FREE FID-gated daily single + signer-issued free vouchers (free daily / one-time free-pack airdrop); $WORD-only at launch (an ETH auto-swap path exists, deferred). Clamped `priceKeeper` repeg. `upgrade` (Rolls/Words only) burns a lowercase, mints its uppercase — conserving supply. |
